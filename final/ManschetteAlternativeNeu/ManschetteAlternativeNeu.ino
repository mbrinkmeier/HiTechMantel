...
#define PULSE_AVG_COUNT 1
#define PULSE_IBI_COUNT 10
...
int pulseOldValue;             // The prior sample
...
void measurePulse() {
  ...
  // If at least 250 ms passed, no pulse was detected yet and at least 3/5 of the last pulse duration
  ...
  if ( pulseTick > 250 ) {
    if ( ( pulseRaw > pulseMid ) && ( pulse == false ) && ( pulseTick > ( pulseIBI/5 )*3 ) ) {
      ...
      if ( PULSE_IBI_COUNT > 1 ) {
        // pulseRateSum = pulseRateSum - pulseRate[pulseRateIdx];
        pulseRate[pulseRateIdx] = pulseIBI;
        pulseRateSum = 0;
        int pulseIBIMin = 60000;
        int pulseIBIMax = 0;
        for (int i = 0; i < PULSE_IBI_COUNT; i++ ) {
          pulseRateSum += pulseRate[i];
          if ( pulseRate[i] > pulseIBIMax ) pulseIBIMax = pulseRate[i];
          if ( pulseRate[i] < pulseIBIMin ) pulseIBIMin = pulseRate[i];
        }
        pulseRateIdx = (pulseRateIdx+1) % PULSE_IBI_COUNT;
        pulseRateAvg = pulseRateSum / PULSE_IBI_COUNT;
        pulseIBIVar = pulseIBIMax - pulseIBIMin;
      } else {
        pulseRateAvg = pulseIBI;
      }
  ...
  if ( ( (pulseBPM >= 20) && (pulseBPM <= 120) ) ) { 
     ...
     screenSerial.print("pulseText.txt=\"Suche Puls ...\"");
     screenSerial.write(0xff);
     screenSerial.write(0xff);
     screenSerial.write(0xff);      
  }
  for ( int i = 0; i < 5; i++ ) {
    ...
    screenSerial.print(pulseOldValue + i * (pulseValue-pulseOldValue) / 5);
    screenSerial.write(0xff);
    screenSerial.write(0xff);
    screenSerial.write(0xff);
